...
namespace LayerTilesConstants {
  ...
  constexpr float maxCoord = 250.f;
  constexpr float minCoord = -250.f;
  ...
  constexpr int nLines = LayerTilesConstants::ceil((maxCoord-minCoord)/tileSize);
  constexpr int maxTileDepth = 40;

  //constexpr float rX = nColumns/(maxX-minX);
  //constexpr float rY = nRows/(maxY-minY);
  constexpr float rCoord = nLines/(maxCoord-minCoord);
}
